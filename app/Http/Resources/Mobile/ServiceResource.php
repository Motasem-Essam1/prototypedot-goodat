...
class ServiceResource extends JsonResource
{
    ...
    public function toArray($request)
    {
        if($request['latitude'] == 0 && $request['longitude'] == 0)
        {
            ...
            $location = LocationService::haversineGreatCircleDistance(ServiceResource::$latitude, ServiceResource::$longitude, $this->location_lat, $this->location_lng);
        }

        $image = $this->images->first() ? asset($this->images->first()->image_path) : '';

        $customer_review = CustomerReview::query()->where('customer_id', $this->user->id)->where('review_id', $this->id)->where('review_type', 'service')->first();
