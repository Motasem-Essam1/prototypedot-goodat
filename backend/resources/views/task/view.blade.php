...
@section('main-style')
    ...
    <meta property="og:description" content="{{ $task->task_description }}"/>
    {{--    <meta property="og:type" content="Services" />--}}
    <meta property="og:image"
          content="{{count($task->images) > 0 ? $task->images[0]->image_path : asset('assets/img/default/default_no_image.jpg') }}"/>
    <meta property="og:url" content="https://deftat.com/task-view/{{$task->task_slug}}"/>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="ratingModalLabel">Rating Task</h5>
                ...
                <div class="modal-body">
                    <div class="form-group-row mb-3">
                        <label class="fgr-label">Task Quality</label>
                    ...
                    <div class="form-group-row mb-3">
                        <label class="fgr-label">Task Time</label>
                        <div class="d-flex align-items-center">
    ...
    <div class="modal modal-share-container fade" id="shareModal" tabindex="-1" aria-labelledby="shareModalLabel"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="modal-b-title">Share your task with friends and family</div>
                    <div class="modal-b-it d-flex align-items-center">
                        <img src="{{ count($task->images) > 0 ? $task->images[0]->image_path : asset('assets/img/default/default_no_image.jpg') }}"
                             alt="">
                        <div class="modal-b-it-title text-capitalize max-1-line">{{ $task->task_name }}</div>
                    </div>
                    <div class="modal-grid">
                        ...
                        <button class="btn share-linked-card wap-btn" data-name="{{ $task->user->name }}"
                                data-phone="{{ $task->user->phone_number }}">
                    ...
                    <div class="breadcrumb-container flex-between-vh mb-3">
                        <ol class="breadcrumb breadcrumb-list">
                            ...
                            <li class="breadcrumb-item text-capitalize"><a
                                        href="#">{{ $task->category->sub_category_name }}</a></li>
                            <li class="breadcrumb-item text-capitalize active max-1-line"
                                aria-current="page">{{ $task->task_name }}</li>
                        </ol>
                        ...
                        <div class="view-product-details">
                            <div class="likes-container">
                                <button {{ !Auth::check() ? "data-toggle=modal data-target=#loginDialog data-toggle=tooltip data-placement=top" : "" }}  title="{{ Auth::check() ? 'Like/UnLike' : "Please Login" }}"
                                        data-toggle="tooltip" data-placement="top" title="Please login"
                                        id="task_fav_btn_{{ $task->id }}"
                                        class="ns-like {{ Auth::check() ? 'favorite-btn' : '' }} {{ $task->is_like ? 'active' : '' }}"
                                        data-id="{{ $task->id }}" data-type="task">
                                ...
                                <span id="task_fav_count_{{ $task->id }}"
                                      class="likes-count">{{ $task->likes_count }}</span>
                            </div>

                            <!-- Product Row -->
                            <div class="view-product-row">
                                <div class="vpr-top">
                                    <div class="vpd-type mb-1 text-capitalize">{{ $task->category->sub_category_name }}</div>
                                    <div class="title mb-2 text-capitalize">{{ $task->task_name }}</div>
                                    ...
                                    <div class="vpd-price-parent">
                                        <div class="vpd-price">Start
                                            From {{$configurations['value']}}{{ $task->starting_price }}</div>
                                    </div>
                                    <div class="vpd-location flex-center-v mb-3">
                                        <span class="icon-location icon mr-1"></span>
                                        <span id="task_{{ $task->id }}" class="kilometers-distance"
                                              data-selector="#task_{{ $task->id }}" data-lat="{{ $task->location_lat }}"
                                              data-lng="{{ $task->location_lng }}"></span> Away
                                    </div>
                                    ...
                                    <div class="vpd-user-product flex-between-vh mt-4">
                                        <a href="{{ route('provider.profile', $task->user->id) }}"
                                           class="vpd-view-user d-flex align-items-center">
                                            <img class="img-fluid" src="{{ asset($task->user->user_data->avatar) }}"
                                                 alt=""/>
                                            <div class="vpd-user-details">
                                                <div class="vpd-title title">{{ $task->user->name }}</div>
                                ...
                                @if(Auth::check())
                                    @if($task->user->user_data->phone_status)
                                        @if(Auth::user()->user_data->user_type == 'Service Provider')
                                            <div class="vpuc-action mt-5">
                                                <button class="btn btn-bg w-100 add-provider-contact"
                                                        ...
                                                        data-item-type="task"
                                                        data-phone="{{ str_replace('+', '', $task->user->country_code . $task->user->phone_number) }}">
...
@section('custom-script')
    ...
    <script>
        ...
        var review_id = {{ Js::from($task->id) }};//provider id
